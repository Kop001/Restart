...
class ToolContext:
    ...
    # Память разговоров. Не передали — заводим свою по настройкам, как это
    # делает и агент: инструменту всё равно, кто её создал.
    ...
    def __post_init__(self) -> None:
        if self.recall is None:
            self.recall = Recall(
                self.config.recall_file,
                keep_days=self.config.recall_days,
                persist=not self.config.private_mode,
            )
